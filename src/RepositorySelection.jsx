...
export default function RepositorySelection({ login }) {
  ...
  return (
    <>
      ...
      />
      {!result.fetching &&
      !pinnedReposResult.fetching &&
      result.data &&
      pinnedReposResult.data ? (
        <>
              ...
              {pinnedReposResult.data.repositoryOwner.pinnedItems.edges
                ...
                .map(({ node: repo }, index, { length }) => (
                  <li key={repo.id} class="w-full">
                    <a
                      href={`/org/${login}/repo/${repo.name}`}
                      class={`flex justify-center flex-col h-full hover:bg-indigo-100 hover:text-indigo-800 cursor-pointer bg-white text-gray-700 py-4 px-8 border-b border-gray-300 ${
                        index === 0 ? "rounded-t" : ""
                      } ${index === length - 1 ? "rounded-b" : ""}`}
                    >
                      <span class="block font-medium mb-2">{repo.name}</span>
          ...
          <ul class="mx-auto px-2 my-4 w-24 w-full max-w-lg flex flex-col list-none items-center">
            {result.data.repositories.edges.map(
              ({ node: repo }, index, { length }) => (
                <li key={repo.id} class="w-full">
                  <a
                    href={`/org/${login}/repo/${repo.name}`}
                    class={`flex justify-center flex-col h-full hover:bg-indigo-100 hover:text-indigo-800 cursor-pointer bg-white text-gray-700 py-4 px-8 border-b border-gray-300 ${
        ...
        <ul class="mx-auto px-2 my-4 w-24 w-full max-w-lg flex flex-col list-none items-center">
          {[1, 2, 3, 4, 5].map((i, index, { length }) => (
            <li key={i} class="w-full">
              <span
                class={`flex flex-col justify-center h-full bg-white text-gray-700 py-4 px-8 border-b border-gray-300 ${
                  index === 0 ? "rounded-t" : ""
                } ${index === length - 1 ? "rounded-b" : ""}`}
              >
                <span
                  class="inline-block bg-gray-200 h-5 mb-3"
                  style={{ width: `${40 + (i % 3) * 20 + i * 5}px` }}
                />
                <span
                  class="inline-block bg-gray-200 h-3 mb-2"
                  style={{ width: `${120 + (i % 3) * 20 + i * 5}px` }}
                />
                {i % 3 && i % 5 ? (
                  <span
                    class="inline-block bg-gray-200 h-3"
                    style={{ width: `${100 + (i % 3) * 20 + i * 5}px` }}
                  />
                ) : null}
              </span>
            </li>
