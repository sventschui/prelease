...
export default function OrganizationSelection() {
  ...
  const items = useMemo(
    () =>
      result.data &&
      [result.data.viewer].concat(result.data.viewer.organizations.nodes),
    [result]
  );

  return (
    <>
      <h2 class="my-4 text-center text-2xl">Create a release</h2>
        ...
        {!result.fetching && items
          ? items.map((owner, index, { length }) => (
              <li key={owner.id} class="w-full">
                <a
                  href={`/org/${owner.login}`}
                  class={`flex h-full items-center hover:bg-indigo-100 hover:text-indigo-800 cursor-pointer bg-white text-gray-700 py-4 px-8 border-b border-gray-300 ${
                    index === 0 ? "rounded-t" : ""
          ...
          : [1, 2].map((i, index, { length }) => (
              <li key={i} class="w-full">
                <span
                  class={`flex items-center h-full bg-white text-gray-700 py-4 px-8 border-b border-gray-300 ${
                    index === 0 ? "rounded-t" : ""
                  } ${index === length - 1 ? "rounded-b" : ""}`}
                >
                  <span class="block w-8 h-8 bg-gray-200 mr-4 rounded-md" />
                  <span
                    class="block bg-gray-200 h-4"
