            ...
            {result.data.repository.branches.edges.map(
              ({ node: branch }, index, { length }) => (
                <li key={branch.id} class="w-full h-16">
                  <a
                    href={`/org/${login}/repo/${repo}/branch?branch=${branch.name}`}
                    class={`flex items-center h-full hover:bg-indigo-100 hover:text-indigo-800 cursor-pointer bg-white text-gray-700 py-4 px-8 border-b border-gray-300 ${
                      index === 0 ? "rounded-t" : ""
